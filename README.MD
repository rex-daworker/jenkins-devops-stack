...
| **Package**            | Archives `dist/` into a versioned `.tar.gz`, tagged with the Jenkins build number                                   |
| **Publish to Nexus**   | Uploads the versioned artifact to a scoped Nexus repo via `curl`, authenticated with a dedicated Jenkins credential |
...
pipeline {
  ...
  stages {
    ...
    stage('Test') {
      steps { sh 'npm test -- --run || echo "no tests yet"' }
    }
    stage('Build') {
    ...
    stage('Package') {
      steps {
        sh 'tar czf trading-app-${BUILD_NUMBER}.tar.gz -C dist .'
      }
    }
    stage('Publish to Nexus') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'nexus-creds', usernameVariable: 'NEXUS_USER', passwordVariable: 'NEXUS_PASS')]) {
          sh 'curl -u $NEXUS_USER:$NEXUS_PASS --upload-file trading-app-${BUILD_NUMBER}.tar.gz http://nexus:8081/repository/trading-app-releases/'
        }
      }
    }
  }
...
**1. The full stack running (Jenkins, SonarQube, Nexus, Agent via Docker Compose)**
...
**7. Versioned artifact published to Nexus**

![Nexus release artifact](docs/images/08-nexus-release.png)

**8. Full pipeline succeeding on the dedicated build agent (multibranch, auto-triggered)**

![Pipeline running on dedicated agent](docs/images/09-agent-pipeline.png)
...
- [x] **Phase 4 — Release:** publish versioned build artifacts to Nexus
- [x] **Phase 5 — Robustness:** Jenkins Configuration as Code (JCasC), multibranch pipeline with webhook triggers, dedicated build agent
